  ...
  cartList(items) {
    ...
    items.products.forEach((item, index) => {
      if (index <= 0) {
        ...
        if (
          this.cartItem[this.cartItem.findIndex((obj) => obj.pId === item.pId)]
        ) {
          //-- if have
          //-- get data old one
          let currentData =
            this.cartItem[
              this.cartItem.findIndex((obj) => obj.pId === item.pId)
            ];
          //--update qty
